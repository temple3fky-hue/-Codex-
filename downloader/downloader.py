...
class AsyncDownloader:
    ...
    async def _safe_download(self, session: aiohttp.ClientSession, task: DownloadTask) -> None:
        for attempt in range(self.settings.max_retries + 1):
            ...
            try:
                ...
                task.status = "failed"
                task.error = str(exc)
                self.log_cb(f"网络失败({attempt + 1}): {task.media_url} -> {exc}")
    ...
    async def _download_one(self, session: aiohttp.ClientSession, task: DownloadTask) -> None:
        async with self.sem:
            ...
            if self.settings.skip_existing and target.exists():
                task.status = "skipped"
                task.local_path = str(target)
                self.progress_cb(task)
                self.log_cb(f"跳过已存在文件: {task.filename}")
                await insert_history(_history_payload(task, file_hash(target)))
                return
